...
@register.filter
def nutrition_rows(text):
    """Parse "Etiqueta: valor" lines into rows for a nutrition-facts table.

    A line with nothing after the colon (e.g. "Por porción:") is treated as
    a subheading rather than a label/value row.
    """
    rows = []
    if not text:
        return rows
    for line in text.splitlines():
        ...
        value = value.strip()
        if sep and value:
            rows.append({"type": "row", "label": label.strip(), "value": value})
        else:
            rows.append({"type": "heading", "label": line.rstrip(":")})
    return rows
